...
/**
 * Get dominant terrain from an array of terrain types
 * Returns the most difficult terrain type
 */
export function getDominantTerrain(terrainTypes: TerrainType[]): TerrainType {
  if (!terrainTypes || terrainTypes.length === 0) {
    return 'PLAIN';
  }
  
  // Priority order from most difficult to least
  const terrainPriority: TerrainType[] = ['MOUNTAINOUS', 'HILLY', 'VALLEY', 'MIXED', 'PLAIN'];
