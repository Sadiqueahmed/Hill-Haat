...
import { CATEGORY_LABELS, QUALITY_LABELS, NE_STATES, NE_DISTRICTS, ORDER_STATUS_LABELS, Listing } from '@/types';
import { HomeSection } from '@/components/sections/HomeSection';
import { LogisticsSection } from '@/components/sections/LogisticsSection';
import { VerificationForm } from '@/components/verification/VerificationForm';
import { AdminVerificationQueue } from '@/components/verification/AdminVerificationQueue';
import { useGeolocation, formatDistance } from '@/hooks/use-geolocation';
import { ProductCard as ProductCardWithDistance } from '@/components/marketplace/ProductCard';
...
export default function HomePage() {
  ...
  const [activeSection, setActiveSection] = useState(() => {
    ...
    if (typeof window !== 'undefined') {
      ...
      if (section && ['home', 'marketplace', 'orders', 'sell', 'logistics', 'profile', 'admin', 'notifications', 'wishlist', 'settings'].includes(section)) {
        return section;
      }
    }
    return 'home';
  ...
  const [deliveryRadius, setDeliveryRadius] = useState<'5' | '10' | '25' | '50' | 'all'>('all');
  ...
  useEffect(() => {
    ...
    // Also read search query from URL
  ...
  const fetchListings = useCallback(async () => {
    ...
    try {
      ...
      if (userLocation) {
        ...
        if (deliveryRadius !== 'all') {
          params.set('radius', deliveryRadius);
        }
  ...
  }, [selectedCategories, selectedStates, searchQuery, priceRange, showOrganicOnly, showVerifiedOnly, sortBy, userLocation, nearMeEnabled, deliveryRadius]);
  ...
  const fetchCart = useCallback(async () => {
    // First try to load from localStorage as it works for all users
    const localCart = localStorage.getItem('hillhaat_cart');
    if (localCart) {
      try {
        const parsed = JSON.parse(localCart);
        setCart(parsed.items || []);
        setCartSummary(parsed.summary || { itemCount: 0, subtotal: 0, deliveryFee: 50, total: 50 });
      } catch {
        // Ignore parse errors
      }
    }

    // If signed in, try to sync with server
    if (isSignedIn) {
      try {
        const response = await api('/api/cart');
        if (response.data) {
          setCart(response.data);
          setCartSummary(response.summary || { itemCount: 0, subtotal: 0, deliveryFee: 50, total: 50 });
          // Update localStorage with server data
          localStorage.setItem('hillhaat_cart', JSON.stringify({
            items: response.data,
  ...
  }, [isSignedIn, dbUser, fetchOrders, fetchMyListings]);
  ...
  const addToCart = async (listingId: string, quantity: number = 1) => {
    ...
    if (isSignedIn) {
      try {
        ...
        console.error('API cart failed, using local storage:', error);
        // Fall through to local storage
      }
    }

    // Use local storage cart (for non-authenticated users or API failure)
    try {
      const localCart = localStorage.getItem('hillhaat_cart');
      let cartData = localCart ? JSON.parse(localCart) : { items: [], summary: { itemCount: 0, subtotal: 0, deliveryFee: 50, total: 50 } };
      
      const existingIndex = cartData.items.findIndex((item: CartItem) => item.listing.id === listingId);
      if (existingIndex >= 0) {
      ...
      // Recalculate summary
      const subtotal = cartData.items.reduce((sum: number, item: CartItem) => sum + item.listing.price * item.quantity, 0);
      ...
      localStorage.setItem('hillhaat_cart', JSON.stringify(cartData));
  ...
  return (
    <div className="flex flex-col min-h-screen">
        ...
        <div className="container mx-auto px-4 py-3">
          <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-hide">
            {[
              ...
              ...(dbUser?.role === 'ADMIN' ? [{ id: 'admin', label: 'Admin', icon: null }] : []),
            ].map((item) => (
              <Button
                key={item.id}
      ...
      <AnimatePresence mode="wait">
                  ...
                  <Card className="border-0 shadow-md">
                    ...
                    <CardContent className="space-y-4">
                      <div className="grid sm:grid-cols-2 gap-4">
                        <div>
                          <Label>State</Label>
                          <Select defaultValue={dbUser.state}>
                            <SelectTrigger>
                        ...
                        <div>
                          ...
                          <Select defaultValue={dbUser.district}>
                            <SelectTrigger>
                              <SelectValue placeholder="Select district" />
                            </SelectTrigger>
                            <SelectContent>
                              {(NE_DISTRICTS[dbUser.state || 'Arunachal Pradesh'] || []).map((d) => (
                      ...
                      <div>
                        <Label>Address</Label>
                        <Textarea placeholder="Your full address" defaultValue={dbUser.address || ''} />
                      </div>
                      <div>
                        <Label>Pincode</Label>
                        <Input placeholder="Pincode" defaultValue={dbUser.pincode || ''} />
                      </div>
                      <Button className="bg-emerald-600 hover:bg-emerald-700">
                        Save Changes
                      </Button>
        ...
        {/* ADMIN SECTION */}
        {activeSection === 'admin' && dbUser?.role === 'ADMIN' && (
          <motion.div
            key="admin"
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
          ...
          >
            <div className="container mx-auto px-4">
              <div className="mb-6">
                ...
                <p className="text-muted-foreground">Manage verification requests and platform settings</p>
              </div>
              ...
              <div className="max-w-2xl mx-auto">
                <div className="flex items-center justify-between mb-6">
                  <div>
                    ...
                    <p className="text-muted-foreground">Stay updated on your orders and activity</p>
                  </div>
                ...
                {!isSignedIn ? (
                  <Card className="border-0 shadow-md">
                    <CardContent className="p-8 text-center">
                      ...
                      <p className="text-muted-foreground mb-4">Get notified about your orders and messages</p>
                      <SignInButton mode="modal">
                        <Button className="bg-emerald-600 hover:bg-emerald-700">Sign In</Button>
                      </SignInButton>
                    </CardContent>
                  ...
                  <div className="space-y-3">
                    <Card className="border-0 shadow-md bg-emerald-50 border-l-4 border-l-emerald-500">
                      <CardContent className="p-4">
                        <div className="flex gap-4">
                          ...
                          <div className="flex-1">
                            ...
                            <p className="text-sm text-muted-foreground">Your order for Organic Ginger has been confirmed by the seller</p>
                      ...
                      <CardContent className="p-4">
                        <div className="flex gap-4">
                          ...
                          <div className="flex-1">
                            ...
                            <p className="text-sm text-muted-foreground">How was your experience? Leave a review for your recent purchase</p>
              ...
              {!isSignedIn ? (
                ...
                <Card className="border-0 shadow-md">
                  <CardContent className="p-12 text-center">
                    ...
                    <p className="text-muted-foreground mb-4">Start adding products you love by clicking the heart icon</p>
                    <Button 
                      variant="outline" 
                      ...
                      <CardContent className="space-y-4">
                        ...
                        <div className="flex items-center justify-between">
                          <div>
                            ...
                            <p className="text-sm text-muted-foreground">Notify when wishlist items drop in price</p>
                          </div>
                          <Switch />
                        </div>
                        <Separator />
...
function FilterContent({
  ...
  deliveryRadius: '5' | '10' | '25' | '50' | 'all';
  onRequestLocation: () => void;
  onToggleNearMe: (enabled: boolean) => void;
  onRadiusChange: (radius: '5' | '10' | '25' | '50' | 'all') => void;
              ...
              <div className="space-y-2">
                ...
                <div className="grid grid-cols-3 gap-2">
                  {[
                    ...
                    <Button
                      key={option.value}
                      variant={deliveryRadius === option.value ? 'default' : 'outline'}
                      size="sm"
                      className={cn(
                        'text-xs',
                        deliveryRadius === option.value && 'bg-emerald-600 hover:bg-emerald-700'
                      )}
                      onClick={() => onRadiusChange(option.value as typeof deliveryRadius)}
                    >
                      {option.label}
                    </Button>
                  ))}
